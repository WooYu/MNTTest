...
- **UDP Probe**：新增 UDP 发包能力，对齐 `OpenFEC与P2P测试对比_0806.xlsx` 的 UDP/FEC 测试口径，验证云聚通对弱网、丢包、抖动、长尾时延的网络数据面效果。
...
- `OpenFEC与P2P测试对比_0806.xlsx` 测的是 `P2P(基于UDP)`、`FEC+P2P(UDP单/双通道)`、NACK、注入丢包/时延后的数据面效果，不是单纯业务成功率。
...
`OpenFEC与P2P测试对比_0806.xlsx` 的价值主要在测试方法：固定包长、发包数、注入丢包/时延、单/双通道、NACK/FEC，再统计丢包率、RTT、长尾和重传。表格中出现了 `P2P(基于UDP)`、`FEC+P2P(UDP单/双通道)`，但没有看到打洞、NAT、STUN/TURN、穿透成功率等测试项。因此这里应先对齐它的 UDP 数据面口径，而不是完整复刻 P2P 建连过程。
...
- 实时运行页：用于现场判断测试是否有效、当前链路是否正在恶化。
- 结果对比页：用于汇报加速前后改善幅度和结论。
