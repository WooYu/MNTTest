...
final class ProbeAccelCompare {
    static final class Verdict {
        final String label;
        final int subtleColor;
        final int borderColor;

        Verdict(String label, int subtleColor, int borderColor) {
            this.label = label;
    ...
    static String oppositeMode(String modeTag) {
        if (modeTag == null) {
            return null;
        }
        switch (modeTag) {
    ...
    static boolean isAccelMode(String modeTag) {
        return "云聚通加速".equals(modeTag) || "弱网加速".equals(modeTag);
    }

    static boolean matchesHistoryHost(ProbeRunRecord record, ProbeConfig cfg) {
        return record.host != null && cfg.host != null && record.host.equals(cfg.host);
    }

    /** 历史 index 可能缺少 weakNetSummary；此时仅在与当前无弱网模拟时配对。 */
    static boolean matchesHistoryWeakNet(ProbeRunRecord record, ProbeConfig cfg) {
    ...
    static ProbeMetrics metricsFromRecord(ProbeRunRecord record) {
    ...
    static Verdict computeVerdict(ProbeMetrics base, ProbeMetrics accel) {
    ...
    static String formatCompareDelta(String name, double prev, double curr, String unit) {
        if (prev <= 0 && curr <= 0) {
            return name + " ±0" + unit;
        }
        if (prev <= 0) {
    ...
    static String formatLossCompareDelta(double prevRate, double currRate) {
    ...
    static String compareMetricLine(String tag, ProbeMetrics metrics) {
