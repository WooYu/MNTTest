 ...
 */
public class ProbePayloadCodec {
    /** 与 {@link ProbeConstants.Payload#MIN_PACKET_BYTES_B} 同源。 */
    public static final int MIN_PACKET_BYTES = ProbeConstants.Payload.MIN_PACKET_BYTES_B;

    private ProbePayloadCodec() {
    }

    /** 回显端单次 JSON 解析：打戳并带回 runId/seq 等元数据，避免重复 parse。 */
    public static final class EchoStampResult {
        public final byte[] payload;
        public final String runId;
        public final int seq;
        public final long clientSendNs;
        public final long clientSendMs;
        public final boolean probePacket;

    public EchoStampResult(byte[] payload, String runId, int seq, long clientSendNs, long clientSendMs,
                boolean probePacket) {
    ...
    /** 探测端收到回显后的解析结果。 */
    public static final class ParsedEcho {
        public final int seq;
        public final JSONObject jsonAck;
        public final boolean compact;

    public ParsedEcho(int seq, JSONObject jsonAck, boolean compact) {
            this.seq = seq;
    ...
    public static byte[] buildPayload(ProbeConfig config, int seq, long sendNs, long sendMs, boolean vpnActive)
            throws Exception {
    ...
    public static String buildLine(ProbeConfig config, int seq, long sendNs, long sendMs, boolean vpnActive) throws Exception {
    return new String(buildPayload(config, seq, sendNs, sendMs, vpnActive), StandardCharsets.UTF_8) + "\n";
    }

    public static boolean usesCompactPayload(ProbeConfig config) {
    return config.packetBytes < minJsonPayloadBytes(config);
    }

    /** 探测端解析回显 payload；compact 模式不校验 runId。 */
    public static ParsedEcho parseEchoPayload(String text, String expectedRunId) throws Exception {
    ...
    public static byte[] stampEchoServerTimes(byte[] payload, long serverRecvNs, long serverSendNs,
    ...
    public static EchoStampResult stampEchoOnce(byte[] payload, long serverRecvNs, long serverSendNs,
            long serverRecvMs, long serverSendMs) throws Exception {
