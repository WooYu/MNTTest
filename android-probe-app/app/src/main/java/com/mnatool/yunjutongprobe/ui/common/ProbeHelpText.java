...
public class ProbeHelpText {
    ...
    static final String PROPERTIES =
            ...
            + "• 包大小 Bytes：单包目标字节数（1–2000 B）。JSON 探测包用 pad 填充；小于 JSON 骨架长度时自动切换紧凑格式"
            + "（{sendMs},{seq},AAA…，与 Autel 联调 App 兼容，回显端原样转发）。\n"
            + "• 速率 PPS 可调 1–8000；发包数/速率/包大小/超时点击字段弹出数字输入框。\n"
            + "• 端口可留空，按协议使用默认（UDP 9001 / TCP 9002 / MQTT 1883）。\n"
