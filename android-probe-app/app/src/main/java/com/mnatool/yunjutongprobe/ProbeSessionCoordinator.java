 ...
 */
final class ProbeSessionCoordinator {

    /** Runner 回调经 runId 校验后的处置方式。 */
    enum RunnerFinishAction {
        /** runId 不匹配或已结束，丢弃回调。 */
        IGNORE,
        /** 用户取消：回参数页，不导出。 */
        CANCEL_RUN,
    ...
    RunnerFinishAction onRunnerFinished(String runId, boolean responder) {
    ...
    RunnerFinishAction onRunnerFailed(String runId) {
        if (!flow.accepts(runId)) {
            return RunnerFinishAction.IGNORE;
        }
        if (cancelRequested) {
