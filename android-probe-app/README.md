...
App 使用严格的三页流程（左上角步骤徽章标示当前步，点击可看说明）：

1. 在「参数设置」页选择协议、模式并填写目标参数，点击「开始测试」。
2. App 自动进入「运行状态」页；运行期间不能跳到其他页面。点击「停止测试」或按系统返回键时，可「继续测试」「停止并查看结果」或「**取消测试**」（取消不导出、直接回参数页）。
3. 探测端自然完成后停留在运行页，须点「查看测试结果」才导出并进入结果页；用户停止或运行失败则自动进结果页。结果页区分完成、停止和失败；有采样数据时可导出 CSV 与 Summary。
...
- `Count/PPS/Bytes/Timeout`：探测参数区提供两档预设，点一下即填充：
  ...
  - **实验室十万级**：`100000 / 2000pps / 100B / 60000ms`（约 50s 发包；大样本补充档，见执行手册 LAB 补充）
