...
- MQTT 主测：执行手册以双平板验证飞书《测试记录 v2.0》记录 1/2 为主线，场景 R1（中转矩阵）/ R2（弱网矩阵），档位 **实验室十万级 @ 2000pps**；见 `docs/云聚通Probe网络测试执行手册.md`。
